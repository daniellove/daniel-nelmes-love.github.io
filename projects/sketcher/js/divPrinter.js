function sktPrintThis(thisThing) {
	var thisContainer = thisThing.closest('.sktContainer').find('.printableDiv')
	var downloadName = thisContainer.attr('downloadName')
	html2canvas(thisContainer, {
		onrendered: function(canvas) {
			var printThis = canvas.toDataURL();
			download(printThis, downloadName, "image/png");
		},
		allowTaint: true,
		taintTest: false
	});
...
function updateSketcherCache(thisThing) {
	var thisCache = thisThing.attr('useobj')
	html2canvas(thisThing, {
		onrendered: function(canvas) {
			sketcherCache[thisCache].push(canvas)
		}
	});
}
